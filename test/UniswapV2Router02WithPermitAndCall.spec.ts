...
describe('PermitAndCall', () => {
  ...
  it('works if allowance has been granted', async () => {
    ...
    const routerFunctionSelector = utils.hexDataSlice(
      utils.id(
        `${routerFunctionName}(${routerFunctionFragment.inputs
          .map(({ type }) => type)
          .join(',')})`,
      ),
      0,
    ...
    const routerFunctionData = utils.defaultAbiCoder.encode(
      routerFunctionFragment.inputs,
      [
        swapAmount,
        constants.Zero,
  ...
  it('works if allowance has not been granted', async () => {
    ...
    const permitSelector = utils.hexDataSlice(
      utils.id(`${permitName}(${permitInputs.join(',')})`),
      0,
      4,
    );
